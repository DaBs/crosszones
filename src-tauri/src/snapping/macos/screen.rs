...
fn rect_intersection(rect1: CGRect, rect2: CGRect) -> CGRect {
  let intersection_bottom_left_point = cmp::max_by(rect1.origin.x, rect2.origin.x, |a, b| a.partial_cmp(b).unwrap());
  let intersection_bottom_right_point = cmp::min_by(rect1.origin.x + rect1.size.width, rect2.origin.x + rect2.size.width, |a, b| a.partial_cmp(b).unwrap());
  let intersection_top_left_point = cmp::max_by(rect1.origin.y, rect2.origin.y, |a, b| a.partial_cmp(b).unwrap());
  let intersection_top_right_point = cmp::min_by(rect1.origin.y + rect1.size.height, rect2.origin.y + rect2.size.height, |a, b| a.partial_cmp(b).unwrap());
  ...
  CGRect::new(&CGPoint::new(intersection_bottom_left_point, intersection_top_left_point), &CGSize::new(intersection_width, intersection_height))
}

fn rect_contained_percentage(rect: CGRect, screen: &DisplayInfo) -> f64 {
  let screen_rect = CGRect::new(&CGPoint::new(screen.x as f64, screen.y as f64), &CGSize::new(screen.width as f64, screen.height as f64));
